...
public class TagServiceTest {
	...
	public void deleteTagsOnPicture() {
		...
		assertTrue("checks return of the service",
				tagService.deleteTagFromPicture(testPic.getId(), deletedTag.getId()));
		Stream<Picture> foundPic = picService.findByTag_id(testPicTags.get(0).getId());
		assertNotEquals("Looks if tag is discounnecet to the picture", testPic.getId(),
				foundPic.findFirst().get().getId());
	...
	public void deleteTagFromList() {
		...
		assertEquals("check if you not able to find pic with deleted Tag", Optional.empty(),
				picService.findPictureByTag(tagToDelete).findFirst());
		...
		assertEquals("Check if all Tags are Delete and not linked to a picture", 0,
				picService.findPictureByTagsOr(tagsToDelete).size());
